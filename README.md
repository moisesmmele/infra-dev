...
2.  **Run Setup Script**:
    Run the setup script to generate SSL certificates and prepare service directories.
    ```bash
    ./setup.sh
    ```
...
1.  **DNS Configuration**: Creates a primary zone in Technitium DNS matching the `DNS_ZONE` variable.
2.  **Proxy Configuration**: Reads environment variables (e.g., `PORTAINER_CONTAINER_NAME`) to automatically create Proxy Hosts in Nginx Proxy Manager (NPM).
    - Uses the `DNS_ZONE` (e.g., `app.example.com`).
    - Configures SSL using the custom certificates generated by `setup.sh`.
